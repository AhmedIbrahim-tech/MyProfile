...
import type { SkillIconComponent } from '@/modules/skills/types';
import {
  FaBroadcastTower,
  FaCheckCircle,
  FaClock,
...
import { NextJsIcon } from '@/shared/components/icons';
...
/** react-icons for Technical Knowledge, Libraries & Familiar With */
export const TECH_LIB_FAMILIAR_ICONS: Record<string, IconType> = {
...
/** Only Next.js needs a custom SVG component */
export const SKILL_ICON_BY_LABEL: Record<string, SkillIconComponent> = {
  'Next.js': NextJsIcon,
};

export const skillIconSvgClass = (skill: string): string => {
...
/**
 * Maps a skill label to its corresponding SVG icon component.
 */
export const getSkillIcon = (skill: string): SkillIconComponent => {
  ...
  if (skillLower.includes('microsoft sql') || skillLower.includes('sql server'))
    return MicrosoftSqlServer;
  if (skillLower.includes('dapper')) return MicrosoftSqlServer;
  if (
    (skillLower.includes('sql') || skillLower.includes('database')) &&
    !skillLower.includes('signal')
  )
    return MicrosoftSqlServer;

  if (
    skillLower.includes('signalr') ||
    skillLower.includes('websocket') ||
    skillLower.includes('real-time')
  )
    return VisualStudio;
  ...
  if (skillLower.includes('fluentvalidation') || skillLower.includes('fluent validation'))
    return Nuget;
  ...
  if (
    skillLower.includes('xunit') ||
    skillLower.includes('moq') ||
    skillLower.includes('unit test')
  )
    return Jest;
